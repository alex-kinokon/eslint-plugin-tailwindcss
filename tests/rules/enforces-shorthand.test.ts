 ...
 */
import rule from "../../src/rules/enforces-shorthand.ts";
...
const skipClassAttributeOptions: Options[] = [
  { skipClassAttribute: true, config: { theme: {}, plugins: [] } },
];

const incompleteCustomWidthHeightOptions: Options[] = [
  {
    config: {
      theme: { extend: { width: { custom: "100px" }, height: { custom: "100px" } } },
      plugins: [],
    },
  },
];

const customSpacingOnlyOptions: Options[] = [
  { config: { theme: { extend: { spacing: { custom: "100px" } } }, plugins: [] } },
];

const customSizeOnlyOptions: Options[] = [
  { config: { theme: { extend: { size: { size: "100px" } } }, plugins: [] } },
];

const ambiguousOptions: Options[] = [
  {
    config: {
      theme: {
        extend: {
...
const generateError = (
  ...
  messageId: string;
  data: { classnames: string; shorthand: string };
} => ({
...
ruleTester.run("shorthands", rule, {
  ...
  invalid: [
    ...
    },
    {
      ...
      `,
      errors: [
        generateError(["rounded-tl-sm", "rounded-tr-sm", "rounded-b-sm"], "rounded-sm"),
      ],
    },
    {
      code: `
    ...
    },
    {
      ...
      options: [{ config: { prefix: "sfc-" } }] satisfies Options[],
    ...
    },
    {
      ...
      errors: [
        generateError(
          ["hover:overflow-hidden", "hover:text-ellipsis", "hover:whitespace-nowrap"],
          "hover:truncate"
        ),
      ],
    },
    {
      ...
      options: [{ config: { prefix: "tw-" } }] satisfies Options[],
    },
    {
      code: `
    ...
    })),
    {
      ...
      options: [{ config: { prefix: "pfx-" } }] satisfies Options[],
    },
    {
      ...
      output:
        '<div class="place-content-center sm:place-items-start md:place-self-end lg:self-start lg:justify-self-center">issue #376</div>',
      errors: [
